...
local function copyDiscord()
    ...
    Library:Notify("Đã sao chép link Discord")
end
...
local function shouldBuySeed(seedType, rarity, mutation)
    ...
    if mode == "Mua tất cả" then
        return true
    elseif mode == "Hạt đã chọn" then
            ...
            if seedType and rarity and shouldBuySeed(seedType, rarity, holder:GetAttribute("Mutation")) then
                ...
                if ok and success then
                    ...
                    if Toggles.BuyNotify.Value then
                        Library:Notify(("Đã mua %s"):format(SeedConfig.SeedDisplayName(seedType)))
                    end
                    if not hasInventorySpace() then
                        return
                    end
    ...
    for _, info in ipairs(wantedFurniture) do
        ...
        if getCoins() - info.price >= reserve then
            ...
            if ok and success and Toggles.FurnitureNotify.Value then
                Library:Notify(("Đã mua %s"):format(info.id))
            end
...
local function doRoundHarvest()
    ...
    if ok and Toggles.HarvestNotify.Value then
        Library:Notify(("Đã thu hoạch với %.2fx"):format(multiplier))
    end
end
    ...
    for _, container in ipairs({ { data.Inventory.Hotbar, true }, { data.Inventory.Storage, false } }) do
        ...
        for slot, item in items or {} do
            if item and item.itemType == "Seed" and item.seedType then
                local allowed = mode == "Bất kỳ hạt" or wantedPlantSeeds[item.seedType] == true
                if allowed then
                    local seed = SeedConfig.GetSeed(item.seedType)
                    local cost = seed and seed.plantCost or 0
                    if mode == "Giá trị cao nhất" then
                        if not best or cost > best then
                            best, bestSlot, bestHotbar = cost, slot, isHotbar
                        end
                    elseif mode == "Giá trị thấp nhất" then
                        if not best or cost < best then
                            best, bestSlot, bestHotbar = cost, slot, isHotbar
                        end
                    else
...
local function doPlant()
    ...
    if ok and started then
        ...
        if Toggles.PlantNotify.Value then
            Library:Notify(("Đã trồng %s"):format(SeedConfig.SeedDisplayName(seedType)))
        end
    end
end
...
local function doPlantTrees()
    ...
    while not Library.Unloaded and Toggles.AutoPlantTrees.Value do
        ...
        if Toggles.PlantNotify.Value then
            Library:Notify("Đã trồng cây đã trưởng thành")
        end
        task.wait(0.2)
    end
end
...
local function doRebirth()
    ...
    if Toggles.RebirthNotify.Value then
        Library:Notify(("Đã Rebirth lên cấp %d"):format(current + 1))
    end
end

local Window = Library:CreateWindow({
    Title = "Ouroboros Hub | Việt hóa",
    Footer = {
...
local Tabs = {
    Info = Window:AddTab("Thông tin", "info"),
    Seeds = Window:AddTab("Hạt giống", "sprout"),
    Farm = Window:AddTab("Nông trại", "trees"),
...
local function AddDiscordButton(Tab)
    ...
    DiscordGroup:AddButton({
        Text = "Vào Discord kiếm tiền",
        Func = copyDiscord,
    })
    DiscordGroup:AddButton({
        Text = "Vào Discord lấy script không key",
        Func = copyDiscord,
    })
end
...
local AccountGroup = Tabs.Info:AddLeftGroupbox("Tài khoản", "circle-user")

AccountGroup:AddLabel(field("Người dùng", LocalPlayer.Name, GREEN), true)
AccountGroup:AddLabel(field("Trạng thái", "Không key", GREEN), true)
AccountGroup:AddLabel(field("Executor", executorName, GREEN), true)

local GameGroup = Tabs.Info:AddLeftGroupbox("Thông tin Game", "gamepad-2")
...
local SessionLabel = GameGroup:AddLabel(field("Thời gian phiên", "0s", ORANGE), true)
...
GameGroup:AddButton({
    Text = "Sao chép script join (Job ID)",
    Func = function()
        ...
        Library:Notify("Đã sao chép script join")
    end,
})
...
task.spawn(function()
    while true do
        ...
        SessionLabel:SetText(field("Thời gian phiên", text, ORANGE))
    end
end)

local ScriptsGroup = Tabs.Info:AddRightGroupbox("Scripts", "package")

ScriptsGroup:AddLabel(colored("Có trong hub này", GREY), true)
ScriptsGroup:AddLabel(colored(GAME_NAME, BLUE), true)

local FeaturesGroup = Tabs.Info:AddRightGroupbox("Tính năng", "list")

FeaturesGroup:AddLabel(colored("Tự động mua hạt & nội thất", BLUE), true)
FeaturesGroup:AddLabel(colored("Tự động trồng & thu hoạch", GREEN), true)
FeaturesGroup:AddLabel(colored("Tự động bán", ORANGE), true)
FeaturesGroup:AddLabel(colored("Tự động Rebirth", GREY), true)

local SocialsGroup = Tabs.Info:AddRightGroupbox("Mạng xã hội", "link")
...
SocialsGroup:AddButton({
    ...
    Func = function()
        ...
        Library:Notify("Đã sao chép link Rscripts")
    end,
})

local AdGroup = Tabs.Info:AddLeftGroupbox("Ouroboros Hub", "sparkles")

AdGroup:AddLabel("Mọi script trong hub đều không key. Không hệ thống key, không checkpoint, không linkvertise.", true)
AdGroup:AddLabel("Discord có sẵn config, cách dupe, giveaway và sớm tiếp cận script mới.", true)
AdGroup:AddLabel("Yêu cầu được xem xét nghiêm túc. Nhiều tính năng trong script này bắt đầu từ tin nhắn Discord.", true)

AdGroup:AddButton({
    Text = "Sao chép link Discord",
    Func = copyDiscord,
})

local FaqGroup = Tabs.Info:AddRightGroupbox("Câu hỏi thường gặp", "circle-help")

FaqGroup:AddLabel("Lấy config tốt ở đâu?", true)
FaqGroup:AddLabel("Vào Discord, kênh config có sẵn config cho mọi script.", true)
FaqGroup:AddLabel("Cách import / export config?", true)
FaqGroup:AddLabel("Vào Discord, hướng dẫn được ghim và mọi người chia sẻ link config hàng ngày.", true)
FaqGroup:AddLabel("Báo lỗi như thế nào?", true)
FaqGroup:AddLabel("Vào Discord và đăng ở kênh bugs.", true)
FaqGroup:AddLabel("Góp ý như thế nào?", true)
FaqGroup:AddLabel("Vào Discord và gửi ở kênh suggestions, hầu hết đều được thêm.", true)
FaqGroup:AddLabel("Nhận hỗ trợ hoặc cập nhật ở đâu?", true)
FaqGroup:AddLabel("Vào Discord, cập nhật và hỗ trợ được đăng ở đó trước.", true)

local BuyGroup = Tabs.Seeds:AddLeftGroupbox("Tự động mua hạt", "shopping-cart")

BuyGroup:AddToggle("AutoBuySeeds", {
    Text = "Tự động mua hạt",
    Default = false,
})

BuyGroup:AddDropdown("BuyMode", {
    Text = "Chế độ mua",
    Values = { "Độ hiếm tối thiểu", "Hạt đã chọn", "Mua tất cả" },
    Default = "Độ hiếm tối thiểu",
    Multi = false,
})

BuyGroup:AddDropdown("BuyMinRarity", {
    Text = "Độ hiếm tối thiểu",
    Values = RARITY_NAMES,
...
BuyGroup:AddToggle("BuyMutatedOnly", {
    Text = "Chỉ mua hạt đột biến",
    Default = false,
})

local BuyLimitGroup = Tabs.Seeds:AddRightGroupbox("Giới hạn", "sliders-horizontal")

BuyLimitGroup:AddInput("BuyMaxCost", {
    Text = "Giá tối đa mỗi hạt (0 = tắt)",
    Default = "0",
    Numeric = true,
    Finished = false,
...
local FurnitureGroup = Tabs.Seeds:AddLeftGroupbox("Tự động mua nội thất", "sofa")

FurnitureGroup:AddToggle("AutoBuyFurniture", {
    Text = "Tự động mua nội thất",
    Default = false,
})

FurnitureGroup:AddDropdown("FurnitureItems", {
    Text = "Nội thất cần mua",
...
local PlantGroup = Tabs.Farm:AddRightGroupbox("Tự động trồng", "sprout")
...
PlantGroup:AddDropdown("PlantMode", {
    Text = "Cách chọn hạt",
    Values = { "Bất kỳ hạt", "Hạt đã chọn", "Giá trị cao nhất", "Giá trị thấp nhất" },
    Default = "Bất kỳ hạt",
    Multi = false,
})
...
for _, key in ipairs(FertilizerConfig.Order) do
    ...
    if fert and (fert.rebirthReq or 0) > 0 then
        PlantGroup:AddLabel(("%s cần Rebirth cấp %d"):format(fert.displayName or key, fert.rebirthReq), true)
    end
end

PlantGroup:AddToggle("PlantDuringWeatherOnly", {
    Text = "Chỉ trồng khi có thời tiết",
    Default = false,
})

PlantGroup:AddDropdown("PlantWeathers", {
    Text = "Thời tiết được phép trồng",
...
PlantGroup:AddToggle("AutoPlantTrees", {
    Text = "Tự động trồng cây đã trưởng thành",
    Default = false,
})

PlantGroup:AddToggle("PlantNotify", {
    Text = "Thông báo khi trồng",
    Default = false,
})
...
local HarvestGroup = Tabs.Farm:AddLeftGroupbox("Tự động thu hoạch", "apple")

HarvestGroup:AddToggle("AutoHarvest", {
    Text = "Tự động thu hoạch",
    Default = false,
})

HarvestGroup:AddInput("HarvestMultiplier", {
    Text = "Thu hoạch tại hệ số",
    Default = "2",
    Numeric = true,
    Finished = false,
    ClearTextOnFocus = false,
...
greenLabel("Ví dụ")
greenLabel("1 = 1x")
greenLabel("1.50 = 1.50x")
greenLabel("0.50 = 0.5x")

HarvestGroup:AddToggle("AutoCollectDead", {
    Text = "Tự động thu cây chết",
    Default = true,
})

HarvestGroup:AddToggle("HarvestNotify", {
    Text = "Thông báo khi thu hoạch",
    Default = false,
})

local FruitGroup = Tabs.Farm:AddLeftGroupbox("Tự động thu trái", "grape")

FruitGroup:AddToggle("AutoCollectFruit", {
    Text = "Tự động thu trái",
    Default = false,
})

FruitGroup:AddToggle("HarvestTeleport", {
    Text = "Dịch chuyển tới từng cây",
    Default = false,
})

FruitGroup:AddToggle("HarvestCollectAll", {
    Text = "Dùng Collect All",
    Default = false,
})

FruitGroup:AddSlider("HarvestActionDelay", {
    Text = "Độ trễ thu thập",
    Default = 0.1,
    Min = 0.05,
    Max = 2,
    Rounding = 2,
...
local SellGroup = Tabs.Farm:AddRightGroupbox("Tự động bán", "banknote")

SellGroup:AddToggle("AutoSellFruits", {
    Text = "Tự động bán trái",
    Default = false,
})

SellGroup:AddToggle("AutoSellAll", {
    Text = "Tự động bán tất cả",
    Default = false,
})

SellGroup:AddToggle("SellDeadTreesOnly", {
    Text = "Chỉ bán cây chết",
    Default = false,
})

SellGroup:AddToggle("SellTeleport", {
    Text = "Dịch chuyển tới quầy bán",
    Default = false,
})

SellGroup:AddSlider("SellMinFruits", {
    Text = "Số trái tối thiểu",
    Default = 1,
    Min = 1,
    Max = 50,
    Rounding = 0,
...
local RebirthGroup = Tabs.Farm:AddRightGroupbox("Tự động Rebirth", "refresh-cw")

RebirthGroup:AddToggle("AutoRebirth", {
    Text = "Tự động Rebirth",
    Default = false,
})

RebirthGroup:AddSlider("RebirthMaxLevel", {
    Text = "Cấp Rebirth tối đa",
...
MenuGroup:AddLabel("Phím menu"):AddKeyPicker("MenuKeybind", {
    Default = "RightShift",
    NoUI = true,
    Text = "Phím mở menu",
})
...
MenuGroup:AddButton("Gỡ script", function()
    Library:Unload()
end)
...
Library:Notify("Greedy Growers đã tải xong")
